...
它面向需要高频阅读论文的研究者：既有本地论文管理器的速度和手感，也有基于真实 PDF 内容的对话、可回跳到原文区域的引用，以及不依赖托管后端的本地 arXiv 发现流程。
...
- **本地 arXiv 发现**：直接读取 arXiv 元数据，缓存 feed、PDF 和缩略图，并把论文保存到本地文库。
- **Codex 增强处理**：为 Discover 结果生成中文标题、总结、贡献说明、标签和相关链接。
- **相似度排序**：可选地用 OpenAI-compatible embedding provider，将 arXiv 结果按本地文件夹或标签相似度排序。
- **本地优先存储**：当前版本不需要 Paper Codex 账号、云同步或产品后端 API。
...
图像生成成功后，Paper Codex 会把生成资产复制进当前 session 工作区，并直接渲染在对话中。点击缩略图即可在应用内打开可缩放预览，不需要跳出阅读器去打开图片文件。
...
阅读器把 PDF 和对话放在同一个工作面板里。PDF 中的选区会成为下一轮提问的一部分，Codex 可以基于真实论文工作区回答，而不是只处理一段松散粘贴的文本。
...
Codex 运行不会被藏在远程服务里。会话文件保存在磁盘上，每轮交互都会保留 prompt 上下文、输出日志和本地工作区产物。
...
- OCR/扫描版 PDF 还不是主要目标；高质量锚定需要 PDF 有可用文本层。
- 云同步、账号和多设备状态暂时不在当前版本范围内。
