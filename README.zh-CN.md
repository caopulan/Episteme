<p align="center">
  <img src="docs/assets/logo-rounded.png" alt="Episteme 标志" width="132">
</p>
...
Episteme 是一个原生 macOS 论文工作台，用来阅读、整理和讨论学术论文。PDF、文件夹、标签、笔记、arXiv 缓存、缩略图、阅读会话和生成结果都保存在本机；当你需要 AI 研究助手时，它会在具体论文的本地工作区里调用 Codex CLI。

它面向需要高频阅读论文的研究者：既有本地论文管理器的速度和手感，也有基于真实 PDF 内容的对话、可回跳到原文区域的引用，以及以本机缓存为中心的 arXiv 发现流程。

## 介绍视频

观看带动态 UI 聚焦镜头的详细版 Remotion 产品介绍视频：[docs/assets/videos/episteme-intro.mp4](docs/assets/videos/episteme-intro.mp4)。
...
- 🔎 **原文锚定对话**：选择 PDF 中的文本后向本地 agent runtime 提问，回答中的引用可绑定到原始页面区域。
- 🧰 **Agent 会话工作区**：每个会话都会写入本地工作区，包含 PDF、元数据、锚点、抽取文本和 turn 日志。
- 🎨 **生成图像支持**：imagegen 生成结果可以直接出现在对话中，并支持应用内放大预览。
- 🔭 **本地 arXiv 探索**：直接读取 arXiv 元数据，缓存 feed、PDF 和缩略图，并把论文保存到本地文库。
- ✨ **Agent 增强处理**：为探索结果生成中文标题、总结、贡献说明、标签和相关链接。
...
- [Codex CLI](https://github.com/openai/codex)，用于 Codex 对话、增强处理和图像生成工作流
- 可选的本地 agent CLI runtime，包括 Kimi ACP（`kimi acp`）和 Gemini ACP（`gemini --experimental-acp`）
...
图像生成成功后，Episteme 会把生成资产复制进当前 session 工作区，并直接渲染在对话中。点击缩略图即可在应用内打开可缩放预览，阅读流程保持在同一个窗口里。

## 日常工作流

### 整理论文文库

Episteme 会在本地保存 PDF 和元数据。你可以用文库侧栏的文件夹树组织论文，再通过搜索、文件夹范围和阅读动作收窄当前列表。
...
该脚本会验证 Codex、Claude Code、原生 Kimi CLI、Kimi ACP、Gemini ACP 和 OpenClaw Kimi 路由是否能看到 `workspace_manifest.json`、Episteme 引用契约，以及 app 正在运行时的 live MCP endpoint。默认只读，不会修改文库论文、文件夹、标签或笔记。

如果 OpenClaw Kimi 被当前本地账号或会员状态挡住，可以使用原生 Kimi CLI 路由、ACP 路由或已配置的 Hermes Kimi 路由：
...
如果要让 Episteme 作为本机 Codex 的插件出现，打开 Settings -> Episteme MCP，点击 **Install / Update**。app 会写入本地 Codex marketplace、plugin cache、MCP endpoint 配置和 Episteme skills。后续 app 端口变化或插件/skill 内容更新时，仍然通过同一个按钮刷新；已安装的插件也会在 Episteme 启动时自动刷新。
...
Episteme 由 SwiftUI macOS 外壳和本地核心库组成。
...
- Agent runtime adapters 会把 Codex、Claude Code、Kimi ACP / Gemini ACP 等 ACP agent、Hermes、OpenClaw Kimi 和 pi 接到同一个 session workspace 契约上。
...
如需接入更多 ACP runtime，可以在支持目录写入 `agent-runtimes.json`，也可以用 `EPISTEME_AGENT_RUNTIMES_PATH` 指向另一个 JSON 文件：
...
已有开发脚本和本地配置里的 `PAPER_CODEX_*` 环境变量仍然兼容。
...
Episteme 会在 `PATH` 和常见安装位置中查找 `codex`，包括 Codex app bundle 和 Homebrew 路径。
...
当前仓库尚未包含开源许可证文件。在把 Episteme 作为开源软件依赖或分发前，请先添加 `LICENSE` 文件。
